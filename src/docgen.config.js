...
module.exports = {
  ...
  outDir: 'src/content/components', // folder to save components docs in (relative to the current working directry)
  apiOptions: {
  ...
  getDestFile: (file, config) => {
    return (
      path
        // .join(config.outDir, file.split('/').pop())
        .join(config.outDir, file)
        .replace(/\.vue$/, '.md')
    )
  },
  // output md file
